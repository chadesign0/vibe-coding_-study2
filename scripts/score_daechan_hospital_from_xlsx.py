# -*- coding: utf-8 -*-
"""
대찬병원 키워드 엑셀: 지역·키워드 열만 읽고 네이버 API로 재채점 후 scoring-data.json 에 병합.

- 병합 슬롯은 (monthLabel, hospitalName) 이므로 다른 병원 데이터와 섞이지 않음.
- 엑셀 경로: DAECHAN_KEYWORD_XLSX (미설정 시 기본 경로 사용)
- 월 라벨: DAECHAN_MONTH_LABEL (예: 3월) 또는 파일명에서 N월 자동 추론
"""
...
DEFAULT_XLSX = r"y:\@컨텐츠팀\D_대찬병원\03. 월간보고서\2026년\202603\대찬병원_키워드배점표_2월_20260316.xlsx"
SHEET_KEYS = ("regional-pc", "regional-mob", "national-pc", "national-mob")


def resolve_month_label(xlsx: Path) -> str:
    env = (os.getenv("DAECHAN_MONTH_LABEL") or "").strip()
    if env:
        return env
...
def sheet_key_from_excel_name(name: str) -> str | None:
    """
    시트명 규칙이 병원마다 달라서 폭넓게 인식:
    - '전국' 포함 => national
    - 그 외 => regional
    - '모바일' 또는 MOB 포함 => mob, 아니면 pc
    """
...
def main() -> None:
    xlsx = Path((os.getenv("DAECHAN_KEYWORD_XLSX") or DEFAULT_XLSX).strip())
    if not xlsx.exists():
        raise SystemExit(f"엑셀을 찾을 수 없습니다: {xlsx}")
    print("병합 슬롯: monthLabel =", resolve_month_label(xlsx), "| hospitalName = 대찬병원")
    ...
    print("완료: scoring-data.json 병합, last-run-evidence.json 의 byHospital['대찬병원'] 갱신")
